...
- **WHEN** `nightshift init` runs dependency detection and both `qsv --version` and `flock --version` succeed
...
- **WHEN** `nightshift init` runs dependency detection and `qsv --version` fails (command not found or times out)
...
- **WHEN** `nightshift init` runs dependency detection and `flock --version` fails (command not found or times out)
...
- **WHEN** `nightshift init` runs dependency detection and both `qsv --version` and `flock --version` fail
...
The system SHALL display a `--- Dependencies ---` section in the summary output of `nightshift init`, positioned between the file list and the next-steps or completion section.
...
The system SHALL implement dependency checking logic in a shared utility module (`src/core/dependencies.ts`) to avoid duplicating the check across commands.
