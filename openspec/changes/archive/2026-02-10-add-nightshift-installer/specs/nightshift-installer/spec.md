...
The system SHALL create or merge the Nightshift agent block into `opencode.jsonc` during `nightshift init`, preserving existing user configuration.

#### Scenario: No existing opencode.jsonc
- **WHEN** `nightshift init` runs and `opencode.jsonc` does not exist
- **THEN** the system SHALL create `opencode.jsonc` with the full Nightshift configuration including the three agent definitions and their permissions

#### Scenario: Existing opencode.jsonc without Nightshift agents
- **WHEN** `nightshift init` runs and `opencode.jsonc` exists with user-defined agents but no `nightshift-manager`, `nightshift-dev`, or `nightshift-qa` entries
- **THEN** the system SHALL add the three Nightshift agent entries to the `agent` block while preserving all existing keys, values, and comments

#### Scenario: Existing opencode.jsonc with stale Nightshift agents
- **WHEN** `nightshift init` runs and `opencode.jsonc` already contains Nightshift agent entries from an older version
- **THEN** the system SHALL update the three Nightshift agent entries to match the current template while preserving all non-Nightshift configuration

#### Scenario: Merge preserves comments
- **WHEN** `opencode.jsonc` contains `//` comments
- **THEN** the merge operation SHALL preserve those comments in the output file
...
#### Scenario: Update re-merges opencode.jsonc
- **WHEN** a user runs `nightshift update` and `opencode.jsonc` exists
- **THEN** the system SHALL re-merge the three Nightshift agent entries using the same merge strategy as `init`, preserving non-Nightshift configuration
...
The system SHALL bundle all Nightshift agent, command, and configuration templates inside the npm package under a `templates/` directory.

#### Scenario: Templates directory structure
- **WHEN** the npm package is inspected
- **THEN** `templates/` SHALL contain `agents/nightshift-manager.md`, `agents/nightshift-dev.md`, `agents/nightshift-qa.md`, `commands/nightshift-create.md`, `commands/nightshift-start.md`, `commands/nightshift-archive.md`, `commands/nightshift-add-task.md`, `commands/nightshift-test-task.md`, `commands/nightshift-update-table.md`, and `opencode.jsonc`
