...
This is an AI agent orchestration framework called **Nightshift** — a batch processing system where a manager agent delegates work to dev agents. The framework is distributed as a TypeScript CLI installer (`nightshift init`) that scaffolds agent and skill files into target projects, plus Markdown templates that define agent behaviors, skills, specifications, and workflows.

**Runtime:** [Claude Code](https://code.claude.com/) — Nightshift ships Claude Code subagents and Skills. The CLI installer (`nightshift init`) writes them into the project.
...
|-------|-------|------|------|-----------------|------------|
| Manager | yes | yes | `qsv*`, `flock*` | `nightshift-dev` only | no |
| Dev | yes | yes | `mkdir*`, `qsv*`, `flock*` | none | optional (commented MCP example in `nightshift-dev.md`) |
